// The code comes from https://codesandbox.io/p/sandbox/stage-presets-gltfjsx-forked-f3k5ls?file=%2Fsrc%2Frealism-effects%2Fv2.js
// The source code https://github.com/0beqz/realism-effects

//import { Pass, Effect, RenderPass, Selection, NormalPass } from 'postprocessing';
//import { ShaderChunk, ShaderLib, UniformsUtils, ShaderMaterial, Uniform, Vector2, Matrix4, Vector3, NoBlending, GLSL3, Clock, Quaternion, WebGLMultipleRenderTargets, NearestFilter, FramebufferTexture, LinearFilter, WebGLRenderTarget, FloatType, DataTexture, RGBAFormat, ClampToEdgeWrapping, LinearMipMapLinearFilter, EquirectangularReflectionMapping, TextureLoader, RepeatWrapping, NoColorSpace, MeshPhysicalMaterial, Color, DepthTexture, RedFormat, Source, Matrix3, SRGBColorSpace, HalfFloatType } from 'three';

const { Effect, RenderPass, Selection, NormalPass } = POSTPROCESSING;

const {
    ShaderChunk, ShaderLib, Uniform, Matrix4, NoBlending, GLSL3, Clock, WebGLMultipleRenderTargets, NearestFilter, FramebufferTexture, LinearFilter, DataTexture, RGBAFormat, ClampToEdgeWrapping, LinearMipMapLinearFilter, EquirectangularReflectionMapping, TextureLoader, RepeatWrapping, NoColorSpace, MeshPhysicalMaterial, DepthTexture, RedFormat, Source, Matrix3, SRGBColorSpace
} = THREE;
...
class TemporalReprojectMaterial extends ShaderMaterial {
    constructor(textureCount = 1) {
        let finalFragmentShader = fragmentShader$5.replace('#include <reproject>', reproject).replace('#include <gbuffer_packing>', gbuffer_packing)
        let definitions = ''
...
var traa_compose = '#define GLSLIFY 1\nuniform sampler2D inputTexture;void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){vec4 accumulatedTexel=linearToOutputTexel(textureLod(inputTexture,vUv,0.));outputColor=vec4(accumulatedTexel.rgb,1.);}' // eslint-disable-line
...
class TRAAEffect extends Effect {
    ...
    update(renderer, inputBuffer) {
        ...
        const noJitterMeshes = getVisibleChildren$1(this._scene).filter((c) => isGroundProjectedEnv(c))
...
class CubeToEquirectEnvPass extends POSTPROCESSING.Pass {
    constructor() {
...
class GBufferPass extends POSTPROCESSING.Pass {
...
class SSGIMaterial extends ShaderMaterial {
    constructor() {
        super({
            ...
            fragmentShader: fragmentShader$4.replace('#include <ssgi_utils>', ssgi_utils).replace('#include <gbuffer_packing>', gbuffer_packing),
            vertexShader,
            blending: NoBlending,
            depthWrite: false,
            depthTest: false,
...
class SSGIPass extends POSTPROCESSING.Pass {
...
var fragmentShader$3 =
    '#define GLSLIFY 1\nvarying vec2 vUv;uniform sampler2D inputTexture;uniform sampler2D depthTexture;uniform sampler2D normalTexture;uniform mat4 projectionMatrix;uniform mat4 projectionMatrixInverse;uniform mat4 cameraMatrixWorld;uniform float radius;uniform float phi;uniform float lumaPhi;uniform float depthPhi;uniform float normalPhi;uniform float roughnessPhi;uniform float specularPhi;uniform vec2 resolution;layout(location=0)out vec4 gOutput0;\n#if textureCount == 2\nuniform sampler2D inputTexture2;layout(location=1)out vec4 gOutput1;\n#endif\n#include <common>\n#include <gbuffer_packing>\n#define luminance(a) pow(dot(vec3(0.2125, 0.7154, 0.0721), a), 0.125)\nMaterial mat;vec3 normal;float depth;float glossiness;struct InputTexel{vec3 rgb;float a;float luminance;float w;float totalWeight;bool isSpecular;};struct Neighbor{vec4 texel;float weight;};void evaluateNeighbor(inout InputTexel inp,inout Neighbor neighbor){if(neighbor.weight<0.01)return;inp.rgb+=neighbor.weight*neighbor.texel.rgb;inp.totalWeight+=neighbor.weight;}void getNeighborWeight(inout InputTexel[textureCount]inputs,inout Neighbor[textureCount]neighbors,inout vec2 neighborUv){\n#ifdef GBUFFER_TEXTURE\nMaterial neighborMat=getMaterial(gBufferTexture,neighborUv);vec3 neighborNormal=neighborMat.normal;float neighborDepth=textureLod(depthTexture,neighborUv,0.0).r;\n#else\nvec3 neighborDepthVelocityTexel=textureLod(normalTexture,neighborUv,0.).xyz;vec3 neighborNormal=unpackNormal(neighborDepthVelocityTexel.b);float neighborDepth=neighborDepthVelocityTexel.a;\n#endif\nif(neighborDepth==1.0)return;float normalDiff=1.-max(dot(normal,neighborNormal),0.);float depthDiff=10000.*abs(depth-neighborDepth);\n#ifdef GBUFFER_TEXTURE\nfloat roughnessDiff=abs(mat.roughness-neighborMat.roughness);float wBasic=exp(-normalDiff*normalPhi-depthDiff*depthPhi-roughnessDiff*roughnessPhi);\n#else\nfloat wBasic=exp(-normalDiff*normalPhi-depthDiff*depthPhi);\n#endif\nfloat sw=exp(-normalDiff*5.);for(int i=0;i<textureCount;i++){vec4 t;float specularFactor=1.;if(isTextureSpecular[i]){t=textureLod(inputTexture2,neighborUv,0.);specularFactor=exp(-glossiness*specularPhi);}else{t=textureLod(inputTexture,neighborUv,0.);}float lumaDiff=abs(inputs[i].luminance-luminance(t.rgb));float lumaFactor=exp(-lumaDiff*lumaPhi);float w=specularFactor*mix(wBasic*lumaFactor,sw,inputs[i].w);w*=inputs[i].w;neighbors[i]=Neighbor(t,w);}}vec3 getNormal(Material mat){\n#ifdef GBUFFER_TEXTURE\nreturn mat.normal;\n#else\nvec3 depthVelocityTexel=textureLod(normalTexture,vUv,0.).xyz;return unpackNormal(depthVelocityTexel.b);\n#endif\n}const vec2 poissonDisk[8]=POISSON_DISK_SAMPLES;void outputTexel(inout vec4 outputFrag,InputTexel inp){inp.rgb/=inp.totalWeight;outputFrag.rgb=inp.rgb;outputFrag.a=inp.a;}void main(){depth=textureLod(depthTexture,vUv,0.).r;if(depth==1.0){discard;return;}InputTexel[textureCount]inputs;float maxAlpha=0.;for(int i=0;i<textureCount;i++){vec4 t;if(i==0){t=textureLod(inputTexture,vUv,0.);}else{t=textureLod(inputTexture2,vUv,0.);}float age=1./(log(pow(t.a,phi*(t.a+10.))+1.+t.a)+1.);InputTexel inp=InputTexel(t.rgb,t.a,luminance(t.rgb),age,1.,isTextureSpecular[i]);maxAlpha=max(maxAlpha,inp.a);inputs[i]=inp;}if(maxAlpha>512.){gOutput0=vec4(inputs[0].rgb,inputs[0].a);\n#if textureCount == 2\ngOutput1=vec4(inputs[1].rgb,inputs[1].a);\n#endif\nreturn;}mat=getMaterial(gBufferTexture,vUv);normal=getNormal(mat);glossiness=max(0.,4.*(1.-mat.roughness/0.25));float roughnessRadius=mix(sqrt(mat.roughness),1.,0.5*(1.-mat.metalness));vec4 random=blueNoise();float r=radius*roughnessRadius*sqrt(random.r)*exp(-maxAlpha*0.01);float angle=random.g*2.*PI;float s=sin(angle),c=cos(angle);mat2 rm=mat2(c,-s,s,c)*r;for(int i=0;i<8;i++){vec2 offset=rm*poissonDisk[i];vec2 neighborUv=vUv+offset;Neighbor[textureCount]neighbors;getNeighborWeight(inputs,neighbors,neighborUv);for(int j=0;j<textureCount;j++)evaluateNeighbor(inputs[j],neighbors[j]);}outputTexel(gOutput0,inputs[0]);\n#if textureCount == 2\noutputTexel(gOutput1,inputs[1]);\n#endif\n}' // eslint-disable-line

function generateDenoiseSamples(texelSize) {
    const sqrt2 = 2 ** 0.5 // by Nvidia ReBLUR, for distribution see: https://www.desmos.com/calculator/abaqyvswem

    let samples = [new Vector2(-1, 0), new Vector2(0, -1), new Vector2(1, 0), new Vector2(0, 1), new Vector2(-0.25 * sqrt2, -0.25 * sqrt2), new Vector2(0.25 * sqrt2, -0.25 * sqrt2), new Vector2(0.25 * sqrt2, 0.25 * sqrt2), new Vector2(-0.25 * sqrt2, 0.25 * sqrt2)]
...
/* eslint-disable camelcase */
const finalFragmentShader$1 = fragmentShader$3.replace('#include <gbuffer_packing>', gbuffer_packing)
...
class PoissionDenoisePass extends POSTPROCESSING.Pass {
    constructor(camera, textures, options = defaultPoissonBlurOptions) {
        ...
        const renderTargetOptions = {
            type: FloatType,
            // using HalfFloatType causes the texture to become darker over time
            depthBuffer: false
        }
...
/* eslint-disable camelcase */
class GBufferDebugPass extends POSTPROCESSING.Pass {
...
class MotionBlurEffect extends Effect {
    constructor(velocityPass, options = defaultOptions$1) {
        ...
        const { fragmentShader, uniforms } = setupBlueNoise(motion_blur) // convert the uniforms from type { uniform: value,... } to type ["uniform", value,...]
...
class AOPass extends POSTPROCESSING.Pass {
...
class TAAPass extends POSTPROCESSING.Pass {
    constructor(camera) {
        super('TAAPass')
...
class SharpnessEffect extends Effect {
    constructor(options = defaultOptions) {
        ...
        super('SharpnessEffect', fragmentShader$1, {
            uniforms: new Map([
...
class SparkleEffect extends Effect {
    constructor(camera, velocityDepthNormalPass) {
        const { uniforms, fragmentShader } = setupBlueNoise(fragShader) // convert uniforms, so we can pass them to Map
...
THREE.GradualBackgroundEffect = GradualBackgroundEffect;
THREE.HBAOEffect = HBAOEffect;
//THREE.LensDistortionEffect = LensDistortionEffect;
THREE.MotionBlurEffect = MotionBlurEffect;
//THREE.PoissonDenoisePass = PoissonDenoisePass;
THREE.SSGIEffect = SSGIEffect;
THREE.SSREffect = SSREffect;
THREE.SharpnessEffect = SharpnessEffect;
THREE.SparkleEffect = SparkleEffect;
THREE.TAAPass = TAAPass;
THREE.TRAAEffect = TRAAEffect;
THREE.TemporalReprojectPass = TemporalReprojectPass;
THREE.VelocityDepthNormalPass = VelocityDepthNormalPass;
THREE.VelocityPass = VelocityPass;